...
class Author(Schema):
    ...
    country_code = EnumField(
        CountryCodes, required=True, attribute="country_code_id"
    )
...
class BookSummary(Schema):
    ...
    status = EnumField(
        BookStatuses, required=True, attribute="status_id"
    )
...
class AuthorDetail(Author):
    books = fields.List(
        fields.Nested(BookSummary), dump_only=True
    )
...
book = Book()
book_summary = BookSummary(many=True)
